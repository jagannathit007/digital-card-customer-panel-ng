...
export class AdminHeaderComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.boardNotificationService.currentBoard$
      ...
      .subscribe((boardId) => {
        console.log('Received board change:', boardId);
  ...
  private checkShouldShowElements(url: string) {
    ...
    const isTeamTaskWithBoardId =
      url.startsWith('/task-management/teamtask?') &&
  ...
  onDocumentClick(event: Event) {
    ...
    if (
      !target.closest('.boards-dropdown-container') &&
      !target.closest('[data-boards-button]')
    ) {
      this.isBoardsDropdownOpen = false;
    }
  }
