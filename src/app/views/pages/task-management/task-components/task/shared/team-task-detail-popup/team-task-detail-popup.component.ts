...
export class TeamTaskDetailPopupComponent implements OnInit, OnDestroy {
  ...
  // Status options
  statusOptions = [
    { value: 'normal', label: 'Normal', color: 'tw-bg-gray-100 tw-text-gray-800' },
    { value: 'ready', label: 'Ready', color: 'tw-bg-blue-100 tw-text-blue-800' },
    { value: 'in progress', label: 'In Progress', color: 'tw-bg-yellow-100 tw-text-yellow-800' },
    { value: 'need review', label: 'Need Review', color: 'tw-bg-purple-100 tw-text-purple-800' },
    { value: 'need rework', label: 'Need Rework', color: 'tw-bg-orange-100 tw-text-orange-800' },
    { value: 'on hold', label: 'On Hold', color: 'tw-bg-red-100 tw-text-red-800' },
    { value: 'blocked', label: 'Blocked', color: 'tw-bg-red-100 tw-text-red-800' },
    { value: 'completed', label: 'Completed', color: 'tw-bg-green-100 tw-text-green-800' }
  ];
  ...
  private loadTaskData(): void {
    ...
    this.task = {
      ...
      description: '<p>Create a comprehensive user authentication system with login, register, and password reset functionality.</p>',
      status: 'in progress',
  ...
  getDueDateDisplay(): { text: string; color: string } {
    if (!this.task.dueDate) {
      return { text: 'Set due date', color: 'tw-bg-gray-100 tw-text-gray-600' };
    }
    ...
    if (dueDateOnly.getTime() === todayOnly.getTime()) {
      return { text: 'Due Today', color: 'tw-bg-green-100 tw-text-green-800' };
    } else if (dueDateOnly.getTime() === yesterdayOnly.getTime()) {
      return { text: 'Due Yesterday', color: 'tw-bg-red-100 tw-text-red-800' };
    } else if (dueDateOnly.getTime() === tomorrowOnly.getTime()) {
      return { text: 'Due Tomorrow', color: 'tw-bg-green-100 tw-text-green-800' };
    } else if (dueDateOnly < todayOnly) {
      // Past date
      const month = dueDate.toLocaleDateString('en-US', { month: 'short' }).toUpperCase();
      const day = dueDate.getDate();
      const year = dueDate.getFullYear() !== today.getFullYear() ? ` ${dueDate.getFullYear()}` : '';
      return { text: `DUE ${month} ${day}${year}`, color: 'tw-bg-red-100 tw-text-red-800' };
    } else {
      // Future date
      const month = dueDate.toLocaleDateString('en-US', { month: 'short' }).toUpperCase();
      const day = dueDate.getDate();
      const year = dueDate.getFullYear() !== today.getFullYear() ? ` ${dueDate.getFullYear()}` : '';
      return { text: `DUE ${month} ${day}${year}`, color: 'tw-bg-green-100 tw-text-green-800' };
    }
  }
  ...
  deleteAttachment(attachmentId: string): void {
    ...
    setTimeout(() => {
      try {
        this.task.attachments = this.task.attachments.filter(a => a._id !== attachmentId);
        this.emitTaskUpdate('attachments', this.task.attachments);
      } catch (error) {
  ...
  private createBackup(field?: string): void {
    if (field) {
      this.backupData[field] = JSON.parse(JSON.stringify((this.task as any)[field]));
    } else {
